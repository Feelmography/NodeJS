...
router.get("/", (req, res) => {
  ...
  mysql.execute(sql, [st_name], (err, students, field) => {
    // res.json(students);
    // 내부에서(Mysql에서 select된 데이터) 만들어진 데이터를
...
router.post("/insert", (req, res) => {
  ...
  mysql.execute(sql, Object.values(student), (err, result, field) => {
     ...
     * 잘 추가되었는지 확인하기 위하여 다시 학생정보 list를
     * 보여줘야 한다
     *
     * 이미 학생정보 list를 보여주는 router를 만들어 두었다
     * 다시 여기에서 list를 SELECT 하여 보여주는 코드를
     * 작성하는 대신
...
router.get("/:st_num/detail", (req, res) => {
  ...
  /**
   * DB SQL을 코딩할 떄 매우 주의할 사항!!!
   * 아래와 같이 문자열을 직접 코딩하여
   * WHERE 절을 만들 경우
   * 예를 들어 S OR 1=1 과 같은 문자열을 st_num 변수에
   * 담아서 전달을 하면 WHERE의 조건이 무력화 되는
   * 명령이 실행된다
   * 만약 DELETE, UPDATE 명령을 수행할 때 이러한 코드를
   * 작성하면 해커에 의해 DB가 바로 손상될 수 있다
   *
 ...
 * DB에서 학생정보를 SELECT 하고
 * st_write로 보내서 input box 에 정보를 표시하기
 */
